...
/**
 * Walks every repo in the config and rebuilds its `worktrees` + `isWorktreeLayout` from the current
 * filesystem state. This is the only place we scan disk for worktrees — every other consumer
 * (`enumerateTargets`, the sidebar) reads from `repo.worktrees` so the placeholder `FolderInfo`
 * (returned before the background sweep has fetched git info) already knows which worktree is the
 * base branch and can hide it.
 *
    ...
    const worktrees = await Promise.all(
        onDisk.map(async (path) => {
            ...
            return {
                ...
                parentTaskPath: existing?.parentTaskPath ?? null,
            };
        }),
    );
    return {
 ...
 * matching repo's `worktrees` without touching disk — saves us a full reconcile (one git info read
 * per existing worktree) on every create. The periodic sweep still catches drift if the user
 * creates worktrees outside agent-storm. Returns the input untouched if the repo isn't in config or
...
export function addWorktreeToConfig(
    ...
    repoPath: string,
    worktreePath: string,
    parentTaskPath: string | null = null,
): Config {
...
function repoEquals(a: Readonly<RepoConfig>, b: Readonly<RepoConfig>): boolean {
    ...
    return a.worktrees.every((worktree) => {
        ...
        return (
            ...
            (other.parentTaskPath ?? null) === (worktree.parentTaskPath ?? null) &&
