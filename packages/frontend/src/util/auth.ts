...
/**
 * Resolves with the stored secret. If none is stored yet, waits until one is set by the auth
 * modal (or any other caller of `setStoredSecret`). On a 401, `clearStoredSecret` resets the
 * state and the next `ensureSecret` call blocks until the user re-enters via the modal.
 */
